...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idcompra)){
			...
			echo $rspta ? "Compra registrado" : "No se pudieron registrar todos los datos del ingreso";
		}
		else {
		}
	break;
	...
	case 'anular':
		$rspta=$ingreso->anular($idcompra);
 		echo $rspta ? "Compra anulado" : "Compra no se puede anular";
	break;

	case 'mostrar':
		$rspta=$ingreso->mostrar($idcompra);
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				"0"=>(($reg->estado=='Aceptado')?'<button class="btn btn-warning" onclick="mostrar('.$reg->idcompra.')"><i class="fa fa-eye"></i></button>'.
 					...
 					'<a target="_blank" href="../reportes/exIngreso.php?id='.$reg->idcompra.'"> <button class="btn btn-info"><i class="fa fa-file"></i></button></a>',
 				"1"=>$reg->fecha,
 				"2"=>$reg->proveedor,
 				"3"=>$reg->usuario,
